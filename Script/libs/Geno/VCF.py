...
def dataframe(filename, large=True):
    ...
    """
    VCF_HEADER = ['CHROM', 'POS', 'ID', 'REF', 'ALT', 'QUAL', 'FILTER', 'INFO', 'FORMAT', filename]
    if large:
    ...
    for i, line in enumerate(lines(filename, VCF_HEADER)):
    ...
    result.drop(columns=['INFO', 'FORMAT', filename], inplace=True)
    return result


def lines(filename, VCF_HEADER):
    ...
    with fn_open(filename) as fh:
        for line in fh:
            if line.startswith('#'):
                ...
                yield parse(line, filename, VCF_HEADER)


def parse(line, filename, VCF_HEADER):
    ...
    formats = fields[9].split(':')
    formats_names = ['GT', 'GQX', 'AD', 'DP', 'VF']
    for i, format in enumerate(formats, 0):
        key = formats_names[i]
        value = format
        # Set the value to None if there is no value.
        result[key] = _get_value(value)
