...
import cfg
import libs.options as options
import libs.inputVald as inputVald
import libs.tools as tools

import libs.Geno.geno as geno
import libs.CNV.cnv as cnv
import libs.RC.summary as summary
...
if __name__ == '__main__':
    ...
    initThread = tools.ThreadWithReturnValue(target = _gui_thread,
                                       name = 'InitThread',
                                       args = (root, initial_information,
                                               text_eve, PATHS,))
    initThread.start()

    textThread = tools.ThreadWithReturnValue(target = _text,
                                       name = 'TextThread',
                                       args = (q, text_eve, root, txt,
                                               initial_information,))
    textThread.start()

    genoThread = tools.ThreadWithReturnValue(target = geno.genoThread,
                                       name = 'GenoThread',
                                       args = (root, initial_information, PATHS,
                                              cfg.MyScreen_Ver, genoCompl, q, txt,))
    genoThread.start()

    cnvThread = tools.ThreadWithReturnValue(target=cnv.cnvThread,
                                      name = 'CNVThread',
                                      args=(root, PATHS, cfg.MyScreen_Ver, curdir,
                                            genoCompl, cnvCompl, q, txt,))
    cnvThread.start()

    summaryThread = tools.ThreadWithReturnValue(target = summary.summaryThread,
                                          name = 'summaryThread',
