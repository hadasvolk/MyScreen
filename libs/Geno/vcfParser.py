...
'''
def logger(out):
    log_file = "{}Parser.log".format(out)
    ...
    parser_logger = logging.getLogger('Parser.log')
...
'''
    ...
    Output: VS2 file
'''
def parserMainFunction(inputFile, outputFile, bamFilesDir, vcfFile):
    ...
    with open(vcfFile, 'r') as csv_file:
        ...
        for row in csv_reader:
            ...
            if ((eachRecord['POS']) == 88713540).any():  # special case for AGID 1476
                eachRecord = eachRecord[eachRecord["ALT"].apply(lambda y: y) == altCand]
            ...
            for i, (index, df) in enumerate(eachRecord.iterrows()):
                ...
                if i + 1 == len(eachRecord):  # reached last record
                    ...
                    # 3 medium CNVs dealt separately because of soft clipped reads
                    medium_cnvs = ['chr1-196716420-TATCCAACTTGTGCAAAAAGATAGA-T', 'chr1-235564867-TGGGAGCCACGAA-T',
                                   'chr10-13699443-C-CCTGGGACTCCAGG']
                    mutation = str(df.iloc[0]) + '-' + str(df.iloc[1]) + '-' + df.iloc[3] + '-' + df.iloc[4]
                    if mutation in medium_cnvs:
                        df = SoftClipsSolution.Parser_AnalysisWithSoftClips(inputFile, bamFilesDir, df, mutation, sampleName)
...
'''
def MAIN_ParserWrapper(input_path, output_path, myscreen_version, bam_files_path=''):
    if bam_files_path == '':
        bam_files_path = re.sub(r'{}_RESULTS.+'.format(myscreen_version), '', input_path)
    if os.path.isdir(input_path):
        logger(os.path.join(input_path + os.sep))
    else:
    ...
    parser_logger.info("**Samples parsing started**")
    parser_logger.info('input path: {}'.format(input_path))
    parser_logger.info('----------------------------------------------------')
    norm_vcf_file = cfg.norm_vcf
    for filename in os.listdir(input_path):
        if filename.endswith(".genome.vcf") and "phased" not in filename:
            input_file = os.path.join(input_path, filename)
            output_file = os.path.join(output_path, filename.replace('.genome.vcf', '.genome.VS2.vcf'))
            parser_logger.info("Parsing sample {}".format(filename))
            parserMainFunction(input_file, output_file, bam_files_path, norm_vcf_file)
            parser_logger.info("Finished parsing sample {} \n".format(filename))


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('vcf_files_dir', help='Directory where the vcf files reside')
    parser.add_argument('output_dir', help='Directory to output of the VS2 files')
    ...
    if args.bams_path:
        ...
        bams_path = ''
    MAIN_ParserWrapper(args.vcf_files_dir, args.output_dir, args.MyScreen_version, bams_path)
