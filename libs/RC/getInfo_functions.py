...
"""
def get_INFO_posRes(i, posResFiltered):
    ...
    if type(gene_CA) == float or type(gene_CA) == np.float64: # EMPTY - Special variant (not included in files).
        ...
        try:
            ...
            logging.info("\tWARNING: Custom Annotation in genotyping results file wrong format for AGID - " + agid)
...
"""
def get_INFO_decon(i, decon_filtered):
...
"""
def get_sampleInfo(extraInfo, sampleSex, sampleSet):
    ...
    if isinstance(extraInfo, pd.DataFrame): # extraInfo file was provided.
        ...
        if len(extraInfo.columns) != extraInfo_COLNUM:
            logging.info("\tWARNING: extraInfo file has wrong number of columns. Please fix file.")
        ...
        if len(total_samples) < len(sampleSet):
            logging.info("\tWARNING: extraInfo file has fewer samples than DECoN results file.")
            print("\tWARNING: extraInfo file has fewer samples than DECoN results file.")
        if len(total_samples) > len(sampleSet):
            logging.info("\tWARNING: extraInfo file has more samples than DECoN results file.")
        ...
        for sam in total_samples:
            ...
            if sampleName not in sampleSet: # Check sample name is valid.
                logging.info("\tWARNING: Uncorrelated sample name between DECoN results file and extraInfo file.")
                logging.info("\tWARNING: Sample " + sampleName + " will not be included in the analysis.")
