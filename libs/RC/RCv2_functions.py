...
genotypes = ['CARRIER', 'CARRIER-Georgian', 'CARRIER-Problem', 'HOM'] # For defineStatus().
    ...
    if (indices == []) and (indices_decon == []): # Sample does not exist in 'positiveResults.tsv' and has no DECoN mutations.
        ...
        for i in indices: # Check if hom, het or both.
            ...
            if Classification in genotypes: # Check if valid mutation.
                ...
                if Genotype in ['het', 'het - With soft-clipped reads']: # Check if exists het.
                    het = True
                if Genotype in ['hom', 'hom - With soft-clipped reads']: # Check if exists hom.
                    hom = True
...
"""
def update_samplesummary(sample_summary, sample, run_name, sampleStatus, posResFiltered, decon_filtered, sampleInfoTable):
    ...
    if (geno_indices==[]) and (cnv_indices==[]): # Sample does not exist in 'positiveResults.tsv' and has no DECoN mutations:
        ...
        for i in geno_indices: # from 'positiveResults.tsv'.
            try:
                CA, gene, agid, mutation, mutation_gDNA, Classification, Genotype, GQX, AVF, RD, ARD, AD, moh, eth, DM, MM = get_INFO_posRes(i, posResFiltered)
            except Exception as e:
...
"""
def createResultSummaries(output, office_version, MyScreen_version, Results_Folder, run_name = None, geno_file = None,
    ...
    logging.info("Starting RCv2_functions session:")
    logging.info("MyScreen version: {}".format(MyScreen_version))
    logging.info("Output directory: {}".format(output_folder))
    logging.info("Office version: {}".format(office_version))
    ...
    logging.info("Extracting data from Genotyping Results file...")
    try:
        ...
        logging.error("ERROR: Genotyping Results file wrong format\n{}".format(e))
        sys.exit(2)
    ...
    for raw_sample in posSamples_raw: # Strip pisces ending.
        try:
            ...
            logging.error("ERROR: strip_name() function failed.\n{}".format(e))
            sys.exit(2)
        posSamples.append(name)
    logging.info("Data extraction successful.")

    # --- CNV results extraction --- #
    logging.info("Extracting data from CNV Results file...")
    try:
        ...
        logging.error("ERROR: CNV results file wrong format\n{}".format(e))
        sys.exit(2)
    posSamples_decon = []
    sampleSet = set()
    sampleSex = {}
    ...
    logging.info("Checking genotyping and CNV files sample names are correlated...")
    for posSamp in posSamples:
        if posSamp not in sampleSet:
            ...
            logging.info("FAILED")
            sys.exit(2)
    logging.info("Genotyping and CNV files sample names are valid.")

    # --- sample info extraction --- #
    logging.info("Extracting data from extraInfo file...")
    try:
        sampleInfoTable = get_sampleInfo(extraInfo, sampleSex, sampleSet)
    except Exception as e:
        print("Error when extracting data from extraInfo file. Please check table format, column names and table contents.")
        logging.error("ERROR: when running get_sampleInfo() function.\n{}".format(e))
        sys.exit(2)
    logging.info("Data extraction successful.")
    ...
    logging.info("Defining each sample's genotypic status...")
    sampleStatus = {}
    for cur_sample in sorted(sampleSet):
        logging.info("\tIn sample " + cur_sample + "...")
    ...
    for cur_sample in sorted(sampleSet): # Iterate over every sample.
        logging.info("\tIn sample " + cur_sample + "...")
        try:
            sample_summary = update_samplesummary(sample_summary, cur_sample, run_name, sampleStatus, posResFiltered, decon_filtered, sampleInfoTable)
        except Exception as e:
            print("Error when trying to summarize sample's " + cur_sample + " results.")
            logging.error("ERROR: when running update_samplesummary() function.\n{}".format(e))
            sys.exit(2)
        logging.info("\tSample " + cur_sample + " done.")
    logging.info("Finished filling out sample summary pd dataframe.")
    print("Results summarization successful.")

    ### REPORT CREATOR ###
    print("Creating Microsoft word reports...")
    logging.info("Creating docx reports...")
    for cur_sample in sorted(sampleSet): # Iterate over every sample.
        logging.info("\tIn sample " + cur_sample + "...")
        try:
            createReports(cur_sample, sampleStatus, posResFiltered, decon_filtered, sampleInfoTable, office, output_folder, MyScreen_version)
        except Exception as e:
            print("Error when trying to create report for sample " + cur_sample + ".")
            logging.error("ERROR: when running createReports() function.\n{}".format(e))
            sys.exit(2)
        logging.info("\tSample " + cur_sample + " done.")
    logging.info("Finished Microsoft word reports.")
    logging.info("Continuing to sample summary excel creation, formatExcel.py and DB_statistics file creation.")
    logging.info("---------------------------")
